...
CREATE TRIGGER Transaccion.calculaSubtotal ON Transaccion.DetalleVenta FOR INSERT, UPDATE, DELETE  
AS
	...
	ELSE
	BEGIN
		SELECT @IDPRODUCTO=idProducto,@IDVENTA=idVenta FROM deleted
	END
BEGIN
...
--Suma de los subtotales de detalle de venta
	SELECT @SUMASUBT = ISNULL(SUM(Subtotal), 0) FROM Transaccion.DetalleVenta WHERE idVenta = @IDVENTA
	UPDATE Transaccion.Venta SET TOTAL = @SUMASUBT WHERE idVenta =  @IDVENTA
END

SELECT * FROM Transaccion.Venta;
DELETE FROM Transaccion.Venta;
SELECT ISNULL(SUM(Subtotal), 0) FROM Transaccion.DetalleVenta WHERE idVenta = 5; 


select * from Transaccion.Venta

CREATE TRIGGER Almacen.calculaSubtotal  ON Almacen.DetalleDevolucion FOR INSERT, UPDATE, DELETE
AS
	DECLARE @IDPRODUCTO BIGINT
	...
	ELSE
	BEGIN
		SELECT @IDPRODUCTO=idProducto,@IDDEVOLUCION=idDevolucion FROM deleted
	END
BEGIN
--obtener el precio delporducto
	SELECT @PRECIO=Precio FROM Almacen.Producto WHERE @IDPRODUCTO=idProducto
...
--Suma de los subtotales de detalle de venta
	SELECT @SUMASUBT = ISNULL(SUM(Subtotal), 0) FROM Almacen.DetalleDevolucion WHERE idDevolucion = @IDDEVOLUCION

	UPDATE Almacen.Devolucion SET Total = @SUMASUBT WHERE idDevolucion =  @IDDEVOLUCION
END

SELECT Consulta.idProducto, Consulta.Nombre, Consulta.Precio, Consulta.Tamaño, Consulta.Descripcion, dv.Cantidad
FROM Transaccion.DetalleVenta dv INNER JOIN (SELECT p.idProducto, tp.Nombre, p.Precio, p.Tamaño, tp.DescripcioN 
FROM Almacen.TipoProducto tp INNER JOIN Almacen.Producto p ON p.idTipoProducto = tp.idTipoProducto) AS Consulta ON Consulta.idProducto = dv.idProducto
WHERE dv.idVenta = 11;

SELECT p.idProducto, tp.Nombre, p.Precio, p.Tamaño, tp.DescripcioN 
FROM Almacen.TipoProducto tp INNER JOIN Almacen.Producto p ON p.idTipoProducto = tp.idTipoProducto;

DROP TABLE Almacen.DetalleDevolucion;
